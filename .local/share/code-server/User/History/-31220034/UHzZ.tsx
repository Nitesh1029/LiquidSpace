...
const MapController: FC<{ selectedMarker: Review | null }> = ({ selectedMarker }) => {
  ...
  useEffect(() => {
    const calculateCenter = () => {
      if (!selectedMarker?.selectedLocations || selectedMarker.selectedLocations.length === 0) {
        return [0, 0] as LatLngTuple;
      }
      const totalLat = selectedMarker.selectedLocations.reduce((sum, location) => sum + location.lat, 0);
      const totalLong = selectedMarker.selectedLocations.reduce((sum, location) => sum + location.long, 0);
