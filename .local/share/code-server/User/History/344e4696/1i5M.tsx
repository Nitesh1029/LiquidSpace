...
export const SearchDrawer = ({
  ...
  const [localSelectedLocations, setLocalSelectedLocations] = useState<Location[]>([]);
  ...
  const handleApplyClick = () => {
    onSelectedLocationsChange(localSelectedLocations);
    if (localSelectedLocations.length > 0) {
      const selectedCoords = localSelectedLocations.map((location) => [
        location.lat,
        location.long,
      ]);
            ...
            <div className="grid grid-cols-2 p-4 gap-2 bg-gray-50 rounded-sm">
              {locations.map((location, index) => (
                <div key={index} className="flex items-center">
                  ...
                  <Button
                    variant={
                      localSelectedLocations.includes(location) ? 'secondary' : 'outline'
                    }
