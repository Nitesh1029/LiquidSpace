...
export default function Page() {
  ...
  const cardStyles = darkMode ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black';

  const buttonStyles = darkMode ? 'bg-white text-black' : 'bg-black text-white';
  ...
  return (
    ...
    >
      {showDialog && (
        <div className="fixed w-1/2 h-1/2 top-0 left-0 right-0 bottom-0 m-auto bg-opacity-50">
      ...
      <Card
        className={`items-center text-left  flex flex-col gap-4 bg-gray-100  p-4 w-[60%] shadow shadow-md shadow-gray-300 ${cardStyles}`}
      >
        <CardHeader>
      ...
      <Button onClick={toggleDarkMode} className={`mt-4 p-2 rounded ${buttonStyles}`}>
        Mode
      </Button>
      <Toaster />
    </div>
